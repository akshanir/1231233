...
class TextBox:
    ...
    def __init__(self, x, y, w, h, text=''):
        ...
        self.text = predict('out.jpg')
        self.txt_surface = FONT.render(text, True, YELLOW)

    def update(self, txt):
        # Resize the box if the text is too long.
        self.text = txt
        self.txt_surface = FONT.render(self.text, True, YELLOW)
...
def main():
    ...
    cbox = CaptchaBox(100, 100, 120, 100)
    noMatchBox = pg.Rect(350, 150, 100, 32)
    matchBox = pg.Rect(450, 150, 100, 32)
    predictionBox = TextBox(350, 200, 140, 32)
    PnoMatchBox = pg.Rect(350, 250, 100, 32)
    PmatchBox = pg.Rect(450, 250, 100, 32)
    done = False
    ...
    while not done:
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN and cbox.resetButton.collidepoint(event.pos):
              predictionBox.update(predict('out.jpg'))
            pg.time.wait(100)
        ...
        predictionBox.draw(screen)
        if predictionBox.text == cbox.solution:
            pg.draw.rect(screen, GREEN, PmatchBox)
        else:
            pg.draw.rect(screen, RED, PnoMatchBox)
        pg.display.flip()
        clock.tick(30)
